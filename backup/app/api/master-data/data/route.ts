...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log('📊 Fetching master_data_circulation data with filters:', { 
      currency, line, year, month, startDate, endDate, filterMode, page, limit 
    })

    // Build base query for filtering
    let baseQuery = supabase.from('master_data_circulation').select('*')
    ...
    const countQuery = supabase.from('master_data_circulation').select('*', { count: 'exact', head: true })
    ...
    console.log(`✅ Found ${result.data?.length || 0} records from master_data_circulation (Page ${page} of ${totalPages})`)
    ...
    console.error('❌ Error fetching master_data_circulation data:', error)
    return NextResponse.json({ 
      success: false,
      error: 'Database error while fetching data',
