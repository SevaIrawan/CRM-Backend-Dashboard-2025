...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log('📊 Fetching withdraw_daily data with filters:', { 
      currency, line, year, month, startDate, endDate, filterMode, page, limit 
    })

    // Build base query for filtering
    let baseQuery = supabase.from('withdraw_daily').select('*')
    ...
    const countQuery = supabase.from('withdraw_daily').select('*', { count: 'exact', head: true })
    ...
    console.log(`📊 Total withdraw_daily records found: ${totalRecords}`)
    ...
    if (result.error) {
      ...
      return NextResponse.json({ 
        success: false, 
        error: 'Database error while fetching withdraw data',
        message: result.error.message 
      }, { status: 500 })
    }

    const totalPages = Math.ceil(totalRecords / limit)
    console.log(`✅ Found ${result.data?.length || 0} withdraw_daily records (Page ${page} of ${totalPages})`)
    ...
    return NextResponse.json({ 
      success: false, 
      error: 'Internal server error while fetching withdraw data' 
    }, { status: 500 })
  }
}
