...
export default function USCSNRCustomersPage() {
  ...
  // Columns to display for SNR customers - updated with Handler column
  const retentionColumns = [
    'line',  // ✅ Brand column (first column)
    'user_name',
    'unique_code',
    'first_deposit_date',
    'last_deposit_date',
    'days_inactive',  // ✅ Absent column after LDD
    'active_days',
    'atv',  // ✅ After active_days
    'pf',   // ✅ After atv
    'deposit_cases',
    'deposit_amount',
    'withdraw_cases',
    ...
    'bonus',
    'net_profit',
    'winrate',  // ✅ After net_profit
    'wd_rate',  // ✅ After winrate
    'tier_name',  // ✅ Tier column after wd_rate
    'status',
    'snr_handler'  // ✅ NEW: Handler column (read-only for SNR)
  ]
  ...
  const getColumnHeader = (column: string): string => {
    const headerMap: { [key: string]: string } = {
      ...
      'withdraw_amount': 'WA',
      'tier_name': 'TIER',
      'snr_handler': 'HANDLER'  // ✅ NEW: Map snr_handler to HANDLER
    }
  ...
  useEffect(() => {
    if (!initialLoadDone && line && year && month) {
      console.log('✅ [SNR Customers] Initial load with defaults:', { line, year, month })
      fetchSNRCustomersData()
  ...
  const fetchSlicerOptions = async () => {
    try {
      ...
      console.log('🔍 [SNR Customers CLIENT] Fetching slicer for user:', currentUser?.username)
      
      const response = await fetch('/api/usc-snr-customers/slicer-options', {
        headers: {
          'x-user': JSON.stringify(currentUser)  // ✅ Send current user for auto-filter
        },
        cache: 'no-store' // ✅ Prevent caching
      })
      const result = await response.json()
      
      console.log('📊 [SNR Customers CLIENT] Slicer API response:', {
      ...
      if (result.success) {
        ...
        console.log('🔍 [SNR Customers CLIENT] Lines from API:', result.data.lines)
        console.log('🔍 [SNR Customers CLIENT] Defaults from API:', result.data.defaults)
        ...
        if (result.data.defaults) {
          ...
          console.log('✅ [SNR Customers] Auto-set to defaults:', result.data.defaults)
        } else if (result.data.lines.length > 0) {
          // Fallback: Auto-set line to first option from API
          const firstOption = result.data.lines[0]
          setLine(firstOption)
          console.log('✅ [SNR Customers] Auto-set line to first option:', firstOption)
        }
      }
    } catch (error) {
      console.error('Error fetching usc-snr-customers slicer options:', error)
    } finally {
      setSlicerLoading(false)
    }
  }

  const fetchSNRCustomersData = async () => {
    // ✅ VALIDATION: Don't fetch if required slicers not set yet
    if (!line || !year || !month) {
      console.log('⏳ [SNR Customers] Waiting for slicers to be set...')
      return
    }
    ...
    try {
      ...
      const response = await fetch(`/api/usc-snr-customers/data?${params}`, {
        headers: {
          'x-user': JSON.stringify(currentUser)  // ✅ Send current user for auto-filter
        }
      })
      ...
      console.error('Error fetching usc-snr-customers data:', error)
      setCustomerRetentionData([])
      setLoading(false)
    }
  }
  ...
  const setDateRangeFromMonth = async (selectedMonth: string, selectedYear: string) => {
    ...
    try {
      ...
      // Note: month-max-date API not needed for SNR, date range calculated locally
      // const response = await fetch(`/api/usc-snr-customers/month-max-date?month=${selectedMonth}&year=${yearValue}&line=${line}`, {
      //   headers: {
      //     'x-user': JSON.stringify(currentUser)
      //   }
      // })
      
      // ✅ For SNR, use calculated lastDay (max date from slicer options will be used if needed)
      let maxDateInDB = lastDay
      // Use max date from slicer options if available
      if (slicerOptions.dateRange.max) {
        const maxDate = new Date(slicerOptions.dateRange.max)
        const lastDayDate = new Date(lastDay)
        if (maxDate < lastDayDate) {
          maxDateInDB = slicerOptions.dateRange.max
        }
      }
      ...
      console.log('✅ [SNR Customers Date Range Auto-Set]:', {
        month: selectedMonth,
        year: yearValue,
  ...
  const handleExport = async () => {
    try {
      ...
      console.log('📤 Starting export for SNR customers data...')
      
      const userStr = localStorage.getItem('nexmax_user')
      const currentUser = userStr ? JSON.parse(userStr) : null
      
      const response = await fetch('/api/usc-snr-customers/export', {
        method: 'POST',
        headers: {
    ...
    <Layout customSubHeader={subHeaderContent}>
      <Frame variant="compact">
        <div className="deposit-container">
          {loading ? (
            <StandardLoadingSpinner message="Loading SNR Customers" />
          ) : customerRetentionData.length === 0 ? (
            <div className="empty-container">
              <div className="empty-icon">📭</div>
              <div className="empty-text">
                No SNR customers found for the selected filters
              </div>
            </div>
          ) : (
            <>
                        ...
                        <thead>
                          ...
                          <tbody>
                            ...
                            .map((column) => {
                              ...
                              // ✅ Special rendering for handler column (badge style, read-only)
                              const isHandler = column === 'snr_handler'
                              ...
                              // ✅ Handler column (badge style, read-only)
                              if (isHandler) {
                                return (
                                  <td key={column} style={{ 
                                    textAlign: 'center',
...
function TransactionHistoryModal({
  ...
  const fetchTransactionHistory = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/usc-snr-customers/transaction-history?${params}`, {
        headers: currentUser ? {
          'x-user': JSON.stringify(currentUser)  // ✅ Send current user for auto-filter
        } : {}
      })
