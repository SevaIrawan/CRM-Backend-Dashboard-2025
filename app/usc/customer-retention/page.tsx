...
export default function USCCustomerRetentionPage() {
  ...
  const setDateRangeFromMonth = async (selectedMonth: string, selectedYear: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/usc-customer-retention/month-max-date?month=${selectedMonth}&year=${yearValue}&line=${line}`, {
        headers: {
          'x-user-allowed-brands': JSON.stringify(allowedBrands)
        }
      })
...
function TransactionHistoryModal({
  ...
  const fetchTransactionHistory = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/usc-customer-retention/transaction-history?${params}`, {
