...
export default function ChurnMemberUSC() {
  // SLICERS STATE
  ...
  // DATA STATES
  const [churnData, setChurnData] = useState<ChurnMemberData[]>([])
  ...
  useEffect(() => {
    const loadSlicerOptions = async () => {
      try {
        ...
        const response = await fetch('/api/usc-churn-member/slicer-options')
        const result = await response.json()

        if (result.success) {
          console.log('🔍 [USC Churn Member] Slicer data received from API:', {
            years: result.data.years,
            monthsCount: result.data.months?.length,
            linesCount: result.data.lines?.length,
          ...
          console.log('✅ [USC Churn Member] Slicers set to defaults - will trigger data fetch')
        } else {
  ...
  // Load churn member data when filters change
  useEffect(() => {
    if (!selectedYear || !selectedMonth || !selectedLine) {
      console.log('⏸️ [USC Churn Member] Skipping fetch - filters not ready:', { selectedYear, selectedMonth, selectedLine })
      return
    }

    console.log('🔄 [USC Churn Member] useEffect triggered - fetching data...', { selectedYear, selectedMonth, selectedLine, page: pagination.currentPage })
    fetchChurnMemberData()
  }, [selectedYear, selectedMonth, selectedLine, pagination.currentPage])

  const fetchChurnMemberData = async () => {
    try {
      setLoading(true)
      console.log('🔄 [USC Churn Member] Fetching churn member data...')
      ...
      const response = await fetch(`/api/usc-churn-member/data?${params}`)
      const result = await response.json()
      
      console.log('📊 [USC Churn Member] API response:', result)
      
      if (result.success) {
        setChurnData(result.data)
        setPagination(result.pagination)
        console.log(`✅ Loaded ${result.data.length} churned members`)
      } else {
      ...
      console.error('Error fetching churn member data:', error)
      setChurnData([])
    } finally {
      setLoading(false)
    }
  ...
  const handleExport = async () => {
    try {
      setExporting(true)
      const response = await fetch('/api/usc-churn-member/export', {
        method: 'POST',
        headers: {
  ...
  const customSubHeader = (
    <div className="dashboard-subheader">
      ...
      <div className="subheader-controls">
        ...
        {/* EXPORT BUTTON - KANAN ATAS */}
        <button
          onClick={handleExport}
          disabled={exporting || loading || churnData.length === 0}
          style={{
            padding: '8px 16px',
            backgroundColor: exporting || loading || churnData.length === 0 ? '#9ca3af' : '#10b981',
            color: 'white',
            border: 'none',
            borderRadius: '8px',
            fontSize: '14px',
            fontWeight: '500',
            cursor: exporting || loading || churnData.length === 0 ? 'not-allowed' : 'pointer',
            transition: 'all 0.2s ease',
            boxShadow: '0 1px 2px rgba(0, 0, 0, 0.05)',
            marginLeft: '16px'
          }}
            ...
            <div style={{ 
              ...
              {loading ? (
                <div style={{ padding: '40px', textAlign: 'center' }}>
                  <div className="animate-spin rounded-full h-12 w-12 border-4 border-blue-200 border-t-blue-600 mx-auto mb-4"></div>
                  <p style={{ color: '#6b7280', fontSize: '14px' }}>Loading churn member data...</p>
                </div>
              ) : churnData.length === 0 ? (
                <div style={{ padding: '40px', textAlign: 'center' }}>
                  <p style={{ color: '#6b7280', fontSize: '14px' }}>No churned members found for selected period.</p>
                </div>
              ) : (
            ...
            <div style={{
              ...
              <div style={{ fontSize: '14px', color: '#6b7280' }}>
                Showing <span style={{ fontWeight: '600', color: '#374151' }}>
                  {churnData.length > 0 ? ((pagination.currentPage - 1) * pagination.recordsPerPage + 1) : 0}
