...
export default function USCChurnMemberPage() {
  ...
  const fetchChurnMemberData = useCallback(async (pageNum: number) => {
    ...
    try {
      ...
      console.error('Error fetching usc-churn-member data:', error)
      setChurnMemberData([])
      setLoading(false)
    }
  ...
  useEffect(() => {
    if (!initialLoadDone && line && year && month && month !== 'ALL' && year !== 'ALL') {
      console.log('✅ [USC Churn Member] Initial load with defaults:', { line, year, month })
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data.defaults) {
          ...
          console.log('✅ [USC Churn Member] Auto-set slicers to defaults:', result.data.defaults)
          ...
          console.log('✅ [USC Churn Member] Auto-set slicers to fallback defaults')
        }
      }
    } catch (error) {
      console.error('Error fetching usc-churn-member slicer options:', error)
    } finally {
  ...
  const handleDaysActiveClick = (userName: string, uniqueCode: string, userkey: string) => {
    console.log('🔍 [USC Churn Member] Opening transaction history for user:', userName, uniqueCode, userkey)
    ...
    <Layout customSubHeader={subHeaderContent}>
      <Frame variant="compact">
        <div className="deposit-container">
          {loading ? (
            <StandardLoadingSpinner message="Loading USC Churned Members" />
            ...
            <div className="empty-container">
              ...
              <div className="empty-text">
                {!month || month === 'ALL' || !year || year === 'ALL' 
                  ...
                  : statusFilter !== 'ALL'
                    ? `No USC churn member data found for status: ${statusFilter === 'NEW MEMBER' ? 'New Depositor' : 'Existing Member'}`
                    : 'No USC churn member data found for the selected filters'}
              </div>
            </div>
          ) : (
            <>
...
function TransactionHistoryModal({
  ...
  const fetchTransactionHistory = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/usc-churn-member/transaction-history?${params}`, {
