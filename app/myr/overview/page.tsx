...
export default function MYROverviewPage() {
  ...
  const [selectedCurrency] = useState('MYR'); // Locked to MYR
    ...
    const loadSlicerOptions = async () => {
      try {
        ...
        try {
          ...
          console.warn('⚠️ [MYR Overview] Failed to parse user data from localStorage:', parseError);
          allowedBrands = null;
        }
        ...
        if (result.success) {
          console.log('🔍 [MYR Overview CLIENT] Slicer data received from API:', {
  ...
  useEffect(() => {
    ...
    if (!initialLoadDone && selectedYear && selectedMonth && selectedLine) {
      console.log('✅ [MYR Overview] Initial load (first time):', { selectedYear, selectedMonth, selectedLine });
  ...
  const loadChartData = async (forceDailyMode?: boolean) => {
    ...
    if (!selectedYear || !selectedLine) {
      console.warn('⚠️ [MYR Overview] Cannot load chart: missing year or line', { selectedYear, selectedLine });
      return;
    }
    // ✅ Daily mode requires month selection
    if (currentDailyMode && !selectedMonth) {
      console.warn('⚠️ [MYR Overview] Cannot load daily chart: month not selected', { currentDailyMode, selectedMonth });
      return;
    }
    ...
    try {
      ...
      console.log(`🔄 [MYR Overview] ========== LOADING CHART DATA ==========`);
      console.log(`🔄 [MYR Overview] Mode: ${period.toUpperCase()}${currentDailyMode ? ` | Month: ${selectedMonth}` : ' | Entire Year'}`);
      console.log(`📋 [MYR Overview] Slicer values:`, {
        selectedYear,
        selectedMonth,
        selectedLine,
      ...
      // ✅ SIMPLE LOGIC: When toggle daily ON, fetch from blue_whale_myr table based on active month slicer
      let apiUrl = `/api/myr-overview/chart-data?line=${selectedLine}&year=${selectedYear}&period=${period}`;
      if (currentDailyMode && selectedMonth) {
        apiUrl += `&month=${selectedMonth}`; // ✅ Send month from slicer - CRITICAL: Use currentDailyMode, not isDailyMode!
      }
      
      console.log('🔗 [MYR Overview] API URL (FINAL):', apiUrl);
      
      console.log(`🔗 [MYR Overview] API URL:`, apiUrl);
      ...
      if (!chartResponse.ok) {
        ...
        console.error('❌ [MYR Overview] HTTP Error:', {
      ...
      console.log('📊 [MYR Overview] Chart API Response:', {
      ...
      console.log('🔍 [MYR Overview] Data selection check:', {
      ...
      if (currentDailyMode === true) {
        ...
        if (!chartResult.dailyData || Object.keys(chartResult.dailyData).length === 0) {
          console.error('❌ [MYR Overview] isDailyMode is TRUE but dailyData is missing or empty!', {
            hasDailyData: !!chartResult.dailyData,
            dailyDataKeys: chartResult.dailyData ? Object.keys(chartResult.dailyData).length : 0,
            hasMonthlyData: !!chartResult.monthlyData,
            chartResult
        ...
        console.log('✅ [MYR Overview] FORCED to use DAILY data (toggle is ON):', {
        ...
        if (!chartResult.monthlyData || Object.keys(chartResult.monthlyData).length === 0) {
          console.error('❌ [MYR Overview] isDailyMode is FALSE but monthlyData is missing or empty!');
          throw new Error('Monthly data not available');
        }
        data = chartResult.monthlyData;
        console.log('✅ [MYR Overview] Using MONTHLY data (toggle is OFF):', {
      ...
      if (!data || Object.keys(data).length === 0) {
        console.error(`❌ [MYR Overview] Data is empty for ${currentDailyMode ? 'daily' : 'monthly'} mode`);
        throw new Error(`No ${currentDailyMode ? 'daily' : 'monthly'} data available`);
      }
      
      console.log(`📊 [MYR Overview] ${currentDailyMode ? 'Daily' : 'Monthly'} data:`, {
      ...
      if (Object.keys(data).length > 0) {
          ...
          console.log('🔍 [MYR Overview] ========== DATA TYPE DETECTION ==========');
          console.log('🔍 [MYR Overview] Data type detection:', {
          ...
          if (currentDailyMode === true && !isDailyData) {
            console.error('❌ [MYR Overview] CRITICAL ERROR: Toggle is ON but data is NOT daily format!', {
          ...
          console.log('🔍 [MYR Overview] Format decision:', {
            currentDailyMode: currentDailyMode,
            isDailyMode: isDailyMode,
            isDailyData: isDailyData,
          ...
          if (finalUseDailyFormat) {
            ...
            console.log('✅ [MYR Overview] Sorted as DAILY dates:', sortedKeys.slice(0, 10));
          } else {
            ...
            console.log('✅ [MYR Overview] Sorted as MONTHLY months:', sortedKeys.slice(0, 10));
          }
            ...
            if (finalUseDailyFormat) {
              ...
              try {
                ...
                if (isNaN(date.getTime())) {
                  ...
                  console.error('❌ [MYR Overview] CRITICAL: Toggle ON but key is not a valid date:', key);
                  ...
                  if (key.includes('December') || key.includes('Dec')) {
                    console.warn('⚠️ [MYR Overview] Key appears to be month name, not date. This should not happen when toggle is ON.');
                  }
                  ...
                  console.error('❌ [MYR Overview] Cannot format as date. Key:', key, 'Toggle ON but data format is wrong!');
                ...
                console.error('❌ [MYR Overview] Error formatting date:', key, e);
          ...
          console.log('✅ [MYR Overview] Category format:', {
        ...
        console.log('✅ [MYR Overview] Setting chart data:', {
        ...
        setTimeout(() => {
          ...
          console.log(`✅ [MYR Overview] Chart data loaded and set successfully (${currentDailyMode ? 'DAILY' : 'MONTHLY'})`);
          console.log('✅ [MYR Overview] Chart data state updated, categories:', preparedChartData.daUserTrend?.categories?.slice(0, 10));
        }, 10);
      } else {
        console.warn('⚠️ [MYR Overview] No data to set for charts');
        setChartsLoaded(true); // Even if no data, mark as loaded
      }

    } catch (error) {
      console.error('❌ [MYR Overview] Error loading chart data:', error);
      console.error('❌ [MYR Overview] Error details:', {
          ...
          <label style={{ 
            ...
            <input
              ...
              onChange={(e) => {
                ...
                console.log('🔄 [MYR Overview] ========== TOGGLE DAILY MODE ==========');
                console.log('🔄 [MYR Overview] Toggle change:', { 
                  from: isDailyMode, 
                  to: newValue,
                  selectedYear,
                  selectedMonth,
                ...
                console.log('✅ [MYR Overview] Toggle Daily Mode updated to:', newValue, '- Waiting for Search button click to reload data');
              }}
              style={{ 
                opacity: 0,
                width: 0,
    ...
    <Layout customSubHeader={customSubHeader}>
      <Frame variant="standard">
        ...
        <div style={{
          ...
          {isLoading && <StandardLoadingSpinner message="Loading MYR Overview" />}
                ...
                comparison: {
                  percentage: formatMoMChange(
                    (() => {
                      ...
                      // Use Math.abs for denominator to handle edge cases (same as calculateMYRMoM)
