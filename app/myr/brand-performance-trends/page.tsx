...
export default function BrandPerformanceTrendsPage() {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/myr-brand-performance-trends/export?${params}`, {
        headers: {
      ...
      a.href = url
      a.download = `brand_performance_trends_myr_${periodBEnd}.csv`
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const res = await fetch('/api/myr-brand-performance-trends/slicer-options', {
          headers: {
            'x-user-allowed-brands': JSON.stringify(allowedBrands)
          },
        ...
        const dataRes = await fetch(`/api/myr-brand-performance-trends/data?${params}`, {
          headers: {
            'x-user-allowed-brands': JSON.stringify(allowedBrands)
          }
        })
  ...
  const handleApplyFilters = async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({ periodAStart, periodAEnd, periodBStart, periodBEnd })
      const res = await fetch(`/api/myr-brand-performance-trends/data?${params}`, {
        headers: {
          'x-user-allowed-brands': JSON.stringify(allowedBrands)
        } 
      })
    ...
    <Layout customSubHeader={customSubHeader} pageInsights={calculateInsights}>
      <Frame variant="standard">
        ...
        <div style={{
          ...
          {loading && <StandardLoadingSpinner message="Loading Brand Performance Trends MYR" />}
            ...
            <>
              ...
              <div className="kpi-row" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '18px', marginBottom: '24px' }}>
                ...
                <ComparisonStatCard
                  ...
                  valueA={formatKPIValue(data.comparison.periodA.depositAmount, 'currency', 'RM')}
                  valueB={formatKPIValue(data.comparison.periodB.depositAmount, 'currency', 'RM')}
                  additionalKpi={{
                    label: "Compare (B-A)",
                    value: formatKPIValue(data.comparison.difference.depositAmount, 'currency', 'RM'),
                ...
                <ComparisonStatCard
                  ...
                  valueA={formatKPIValue(data.comparison.periodA.netProfit, 'currency', 'RM')}
                  valueB={formatKPIValue(data.comparison.periodB.netProfit, 'currency', 'RM')}
                  additionalKpi={{
                    label: "Compare (B-A)",
                    value: formatKPIValue(data.comparison.difference.netProfit, 'currency', 'RM'),
                ...
                <ComparisonStatCard
                  title="DA USER A|B"
                  valueA={formatKPIValue(data.comparison.periodA.daUser, 'currency', 'RM')}
                  valueB={formatKPIValue(data.comparison.periodB.daUser, 'currency', 'RM')}
                  additionalKpi={{
                    label: "Compare (B-A)",
                    value: formatKPIValue(data.comparison.difference.daUser, 'currency', 'RM'),
                ...
                <ComparisonStatCard
                  title="GGR USER A|B"
                  valueA={formatKPIValue(data.comparison.periodA.ggrUser, 'currency', 'RM')}
                  valueB={formatKPIValue(data.comparison.periodB.ggrUser, 'currency', 'RM')}
                  additionalKpi={{
                    label: "Compare (B-A)",
                    value: formatKPIValue(data.comparison.difference.ggrUser, 'currency', 'RM'),
