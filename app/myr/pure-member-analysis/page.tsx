...
export default function MYRPureMemberAnalysisPage() {
  ...
  const fetchSlicerOptions = async () => {
    try {
      ...
      const response = await fetch('/api/myr-pure-member-analysis/slicer-options', {
        cache: 'no-store'
      })
  ...
  const fetchPureMemberData = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/myr-pure-member-analysis/data?${params}`)
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await fetch('/api/myr-pure-member-analysis/export', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        const filename = `myr_pure_member_${metrics}_${year}.csv`
        a.download = filename
    ...
    <Layout customSubHeader={subHeaderContent}>
      <Frame variant="compact">
        <div className="deposit-container">
          {loading ? (
            <StandardLoadingSpinner message="Loading Pure Member Analysis MYR" />
