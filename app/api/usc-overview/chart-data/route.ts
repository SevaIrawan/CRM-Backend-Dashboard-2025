...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn('⚠️ [USC Overview Chart API] Failed to parse user allowed brands header:', parseError);
    ...
    if (period === 'daily') {
      // Query daily data from blue_whale_usc table
      // Note: Some calculated fields (atv, purchase_frequency, da_user, ggr_user, winrate, etc.) 
      // need to be calculated from raw fields
      
      // ✅ CRITICAL: blue_whale_usc table stores month as STRING (month name), not number!
      ...
      console.log('🔄 [USC Overview Chart API] Using month name for daily query:', {
        monthName: monthName,
        year: parseInt(year),
        line
      });
      
      // ✅ SIMPLE LOGIC: Query from blue_whale_usc table based on active month slicer
      // ✅ CRITICAL: Use month name (string) directly, NOT month number!
      // ✅ NOTE: new_register and new_depositor are NOT in blue_whale_usc table
      ...
      if (dailyError) {
        console.error('❌ [USC Overview Chart API] Error fetching daily data:', dailyError);
        console.error('❌ [USC Overview Chart API] Error details:', {
      ...
      if (newRegisterError) {
        console.warn('⚠️ [USC Overview Chart API] Error fetching new_register data (non-critical):', newRegisterError);
      ...
      console.log('📊 [USC Overview Chart API] Raw daily data from blue_whale_usc:', {
      ...
      if (!dailyData || dailyData.length === 0) {
        console.warn('⚠️ [USC Overview Chart API] No daily data found for:', {
          year: parseInt(year),
          month: monthName, // ✅ Use month name, not number
          line: line || 'ALL'
        });
      ...
      try {
        dailyData?.forEach(row => {
          try {
            const dateKey = String(row.date || '').trim();
            if (!dateKey) {
              console.warn('⚠️ [USC Overview Chart API] Row with missing date, skipping:', row);
            ...
            // Note: We can't determine new_depositor from blue_whale_usc alone, so we'll use new_register table data
            ...
            console.error('❌ [USC Overview Chart API] Error processing row:', rowError, row);
            // Continue processing other rows
          }
        });
      } catch (aggregationError) {
        console.error('❌ [USC Overview Chart API] Error during daily data aggregation:', aggregationError);
        return NextResponse.json({
          success: false,
          ...
          try {
            ...
            if (/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) {
              ...
              if (isNaN(date.getTime())) {
                console.warn('⚠️ [USC Overview Chart API] Invalid date key:', dateKey);
                // Skip invalid dates
                return;
              }
            ...
            console.error('❌ [USC Overview Chart API] Error formatting date key:', dateKey, e);
            // Skip this date key if there's an error
          }
        });
      } catch (formatError) {
        console.error('❌ [USC Overview Chart API] Error during date formatting:', formatError);
        return NextResponse.json({
          success: false,
      ...
      if (Object.keys(formattedDailyData).length === 0) {
        console.warn('⚠️ [USC Overview Chart API] No valid daily data after formatting');
        return NextResponse.json({
          success: true,
          dailyData: {},
          monthlyData: null
      ...
      console.log('✅ [USC Overview Chart API] Daily data prepared:', {
    ...
    data?.forEach(row => {
      ...
      if (monthNumber < 1 || monthNumber > 12) {
        console.warn('⚠️ [USC Overview Chart API] Invalid month number:', monthNumber);
