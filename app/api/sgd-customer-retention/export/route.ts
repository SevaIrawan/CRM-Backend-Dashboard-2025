...
export async function POST(request: NextRequest) {
  try {
    ...
    let query = supabase.from('blue_whale_sgd').select('userkey, user_name, unique_code, update_unique_code, date, line, year, month, traffic, first_deposit_date, days_inactive, deposit_cases, deposit_amount, withdraw_cases, withdraw_amount, bonus, add_bonus, deduct_bonus, net_profit')
    ...
    const filename = `sgd_customer_retention_export_${timestamp}.csv`
