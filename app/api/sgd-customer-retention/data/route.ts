...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let baseQuery = supabase.from('blue_whale_sgd').select('userkey, user_name, unique_code, date, line, year, month, first_deposit_date, days_inactive, deposit_cases, deposit_amount, withdraw_cases, withdraw_amount, bonus, add_bonus, deduct_bonus, net_profit')
...
function processCustomerRetentionData(rawData: any[], previousMonthUsers: Set<string>, selectedMonth: string | null, selectedYear: string | null, userMinDates: Map<string, string>) {
  ...
  if (rawData.length > 0) {
    ...
    console.log(`🔍 Sample bonus values:`, {
      bonus: rawData[0].bonus,
      add_bonus: rawData[0].add_bonus,
      deduct_bonus: rawData[0].deduct_bonus,
      bonus_type: typeof rawData[0].bonus,
      add_bonus_type: typeof rawData[0].add_bonus,
      deduct_bonus_type: typeof rawData[0].deduct_bonus
    })
  }
