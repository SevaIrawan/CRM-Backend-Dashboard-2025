...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📥 Exporting MYR churn member data with filters:', { 
      line, year, month,
    ...
    return new NextResponse(csvContent, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="myr_churn_member_${year}_${month}_${line || 'ALL'}_${new Date().toISOString().split('T')[0]}.csv"`
      }
    })

  } catch (error) {
    console.error('❌ Error exporting MYR churn member data:', error)
    return NextResponse.json({
...
async function fetchUserMinDates(rawData: any[], line: string | null, userAllowedBrands: string[] | null): Promise<Map<string, string>> {
  try {
    ...
    if (uniqueUsers.length === 0) {
      console.log('✅ [MYR Churn Member Export] All users have valid first_deposit_date, no MIN date fetch needed')
      return new Map()
    }
    
    console.log(`🔍 [MYR Churn Member Export] Fetching MIN dates for ${uniqueUsers.length} users with NULL first_deposit_date`)
    ...
    if (minDateError) {
      console.error('❌ [MYR Churn Member Export] Error fetching MIN dates:', minDateError)
      return new Map()
    }
    ...
    console.log(`📊 [MYR Churn Member Export] Fetched MIN dates for ${userMinDateMap.size} users`)
    
    return userMinDateMap
  } catch (error) {
    console.error('❌ [MYR Churn Member Export] Error in fetchUserMinDates:', error)
    return new Map()
  }
}
