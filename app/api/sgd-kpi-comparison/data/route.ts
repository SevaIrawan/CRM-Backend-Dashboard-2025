...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('📊 [SGD KPI Comparison] Request params:', {
      line,
      periodAStart,
      periodAEnd,
      periodBStart,
    ...
    const calculatePeriodKPIs = async (startDate: string, endDate: string, selectedLine?: string) => {
      console.log(`🔄 [SGD KPI Comparison] Calculating KPIs for ${startDate} to ${endDate}, line: ${selectedLine}`);
      ...
      if (mvError) {
        console.error('❌ [SGD KPI Comparison] Error fetching MV data:', mvError);
        throw mvError;
      }

      if (!mvData || mvData.length === 0) {
        console.log('⚠️ [SGD KPI Comparison] No data found for period');
        return {
      ...
      if (amError) {
        console.error('❌ [SGD KPI Comparison] Error fetching active members:', amError);
        throw amError;
      }
      ...
      console.log(`✅ [SGD KPI Comparison] Calculated KPIs for period`);
    ...
    console.log('✅ [SGD KPI Comparison] Comparison data calculated successfully');
    ...
    console.error('❌ [SGD KPI Comparison] Error:', error);
    return NextResponse.json(
      { error: 'Failed to calculate SGD KPI comparison', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
