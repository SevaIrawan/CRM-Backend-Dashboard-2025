...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log('🔐 [MYR Overview API] User brand access:', {
    ...
    console.log('🔍 [MYR Overview API] RAW BRANDS FROM DATABASE:', {
      total: uniqueLines.length,
      all_brands: uniqueLines,
      clean_brands: cleanLines
    })
    
    console.log('🔍 [MYR Overview API] USER PERMISSION:', {
      allowed_brands: userAllowedBrands,
      is_squad_lead: userAllowedBrands !== null && userAllowedBrands?.length > 0
    })
    ...
    console.log('🔍 [MYR Overview API] AFTER filterBrandsByUser:', {
      filtered_brands: filteredBrands
    })
    
    let linesWithAll = ['ALL', ...filteredBrands.sort()]
    
    console.log('🔍 [MYR Overview API] AFTER adding ALL:', {
    ...
    console.log('✅ [MYR Overview API] FINAL BRANDS FOR USER:', {
