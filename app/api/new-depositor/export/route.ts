...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📥 Exporting new_depositor_daily data with filters:', { 
      currency, line, year, month, startDate, endDate, filterMode 
    })

    // Build query with same filters as data endpoint
    let query = supabase.from('new_depositor_daily').select('*')
    ...
    console.log(`📊 Exporting ${data.length} new_depositor_daily records`)
    ...
    const filename = `new_depositor_daily_data_${filterStr}_${timestamp}.csv`
