...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🔍 [USC Member Report API] RAW brands from database:', lines)
    console.log('🔍 [USC Member Report API] User allowed_brands:', userAllowedBrands)
    ...
    if (userAllowedBrands && userAllowedBrands.length > 0) {
      ...
      console.log('🔐 [USC Member Report API] Squad Lead - filtered brands:', finalLines)
    } else {
      // Admin/Manager/SQ: Add 'ALL' + all brands
      finalLines = ['ALL', ...lines.sort()]
      console.log('✅ [USC Member Report API] Admin/Manager - ALL + brands:', finalLines)
    }
