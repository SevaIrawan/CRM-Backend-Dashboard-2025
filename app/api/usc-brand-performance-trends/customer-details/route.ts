    ...
    const customerDetails = await Promise.all(
      uniqueUserKeys.map(async (userkey) => {
        ...
        let transactionsQuery = supabase
          .from('blue_whale_usc')
          .select('userkey, user_name, unique_code, date, line, deposit_amount, withdraw_amount, deposit_cases, withdraw_cases, net_profit, ggr, valid_amount, bonus, add_transaction, deduct_transaction')
          .eq('currency', 'USC')
          .eq('userkey', userkey)
          .gte('date', startDate)
          .lte('date', endDate)
