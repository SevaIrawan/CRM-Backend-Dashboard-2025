...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('📊 [USC KPI Comparison] Request params:', {
      line,
      periodAStart,
      periodAEnd,
      periodBStart,
    ...
    const calculatePeriodKPIs = async (startDate: string, endDate: string, selectedLine?: string) => {
      console.log(`🔄 [USC KPI Comparison] Calculating KPIs for ${startDate} to ${endDate}, line: ${selectedLine}`);
      ...
      if (mvError) {
        console.error('❌ [USC KPI Comparison] Error fetching MV data:', mvError);
        throw mvError;
      }

      if (!mvData || mvData.length === 0) {
        console.log('⚠️ [USC KPI Comparison] No data found for period');
        return {
      ...
      if (amError) {
        console.error('❌ [USC KPI Comparison] Error fetching active members:', amError);
        throw amError;
      }
      ...
      console.log(`✅ [USC KPI Comparison] Calculated KPIs for period`);
    ...
    console.log('🔄 [USC KPI Comparison] Calculating Period A KPIs...');
    const periodAData = await calculatePeriodKPIs(periodAStart, periodAEnd, line === 'ALL' ? undefined : line);

    console.log('🔄 [USC KPI Comparison] Calculating Period B KPIs...');
    ...
    const metricsConfig = [
      ...
      { key: 'withdrawCases', label: 'Withdraw Cases', type: 'integer', inverse: false },
      { key: 'withdrawAmount', label: 'Withdraw Amount', type: 'amount', inverse: false },
    ...
    console.log('✅ [USC KPI Comparison] Comparison data calculated successfully');
    ...
    console.error('❌ [USC KPI Comparison] Error:', error);
    return NextResponse.json(
