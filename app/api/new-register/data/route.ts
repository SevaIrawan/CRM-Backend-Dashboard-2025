...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log('📊 Fetching new_register data with filters:', { 
      currency, line, year, month, startDate, endDate, filterMode, page, limit 
    })

    // Build base query for filtering
    let baseQuery = supabase.from('new_register').select('*')
    ...
    const countQuery = supabase.from('new_register').select('*', { count: 'exact', head: true })
    ...
    console.log(`📊 Total new_register records found: ${totalRecords}`)
    ...
    if (result.error) {
      ...
      return NextResponse.json({ 
        success: false, 
        error: 'Database error while fetching new_register data',
        message: result.error.message 
      }, { status: 500 })
    }

    const totalPages = Math.ceil(totalRecords / limit)
    console.log(`✅ Found ${result.data?.length || 0} new_register records (Page ${page} of ${totalPages})`)
    ...
    return NextResponse.json({ 
      success: false, 
      error: 'Internal server error while fetching new_register data' 
    }, { status: 500 })
  }
}
