...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log('📊 [SGD Churn Member] Fetching transaction history for user:', {
    ...
    console.log('📊 [SGD Churn Member] Transaction history for previous month:', { prevYear, prevMonth, prevYearInt })
    ...
    if (error) {
      console.error('❌ [SGD Churn Member] Supabase query error:', error)
      return NextResponse.json({
        success: false,
    ...
    const processedData = (data || []).map((row: any) => {
      ...
      if (!firstDepositDate || firstDepositDate === null || firstDepositDate === '') {
        ...
        if (userMinDate) {
          console.log(`🔄 [SGD Churn Member] Transaction ${row.date}: first_deposit_date NULL → fallback to MIN date (${userMinDate})`)
        }
      }
    ...
    console.log(`📊 [SGD Churn Member] Transaction history found: ${processedData.length} transactions`)
    ...
    console.error('❌ [SGD Churn Member] Error fetching transaction history:', error)
    return NextResponse.json({
      success: false,
...
async function fetchUserMinDate(userkey: string, line: string | null, userAllowedBrands: string[] | null): Promise<string | null> {
  try {
    console.log(`🔍 [SGD Churn Member] Fetching MIN date for user: ${userkey}`)
    ...
    if (minDateError) {
      console.error('❌ [SGD Churn Member] Error fetching MIN date:', minDateError)
      return null
    }
    
    if (minDateData && minDateData.length > 0) {
      const minDate = minDateData[0].date as string
      console.log(`📊 [SGD Churn Member] MIN date found for user ${userkey}: ${minDate}`)
      return minDate
    }
    
    console.log(`⚠️ [SGD Churn Member] No MIN date found for user ${userkey}`)
    return null
  } catch (error) {
    console.error('❌ [SGD Churn Member] Error in fetchUserMinDate:', error)
    return null
  }
}
