...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get unique lines (lock to SGD table)
    ...
    console.log('🔍 [SGD Member Report API] RAW brands from database:', lines)
    console.log('🔍 [SGD Member Report API] User allowed_brands:', userAllowedBrands)
    ...
    if (userAllowedBrands && userAllowedBrands.length > 0) {
      ...
      console.log('🔐 [SGD Member Report API] Squad Lead - filtered brands:', finalLines)
    } else {
      // Admin/Manager/SQ: Add 'ALL' + all brands
      finalLines = ['ALL', ...lines.sort()]
      console.log('✅ [SGD Member Report API] Admin/Manager - ALL + brands:', finalLines)
    }
    ...
    console.error('❌ Error fetching blue_whale_sgd slicer options:', error)
    return NextResponse.json({ 
      success: false, 
      error: 'Internal server error while fetching blue_whale_sgd slicer options' 
    }, { status: 500 })
  }
}
