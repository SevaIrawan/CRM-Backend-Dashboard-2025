...
export async function GET(request: NextRequest) {
  ...
  console.log('📊 [USC Churn Member] Raw params:', { line, year, month, page, limit, yearType: typeof year, monthType: typeof month })
  ...
  try {
    ...
    if (!month || month === 'ALL' || !year || year === 'ALL') {
      console.error('❌ [USC Churn Member] ERROR 400: Year and Month must be specific (not ALL)')
    ...
    console.log('📊 [USC Churn Member] Calculating churn members:', { line, year, month })
    
    const yearInt = parseInt(year)
    
    if (isNaN(yearInt)) {
      console.error('❌ [USC Churn Member] ERROR 400: Invalid year format')
      return NextResponse.json({
        success: false,
        error: 'Invalid year',
    ...
    if (currentMonthIndex === -1) {
      console.error('❌ [USC Churn Member] ERROR 400: Invalid month')
      return NextResponse.json({
        success: false,
        error: 'Invalid month',
    ...
    if (isNaN(prevYearInt)) {
      console.error('❌ [USC Churn Member] ERROR 400: Invalid previous year')
      return NextResponse.json({
        success: false,
    ...
    console.log('📊 [USC Churn Member] Previous month calculated:', { prevYear, prevMonth, prevYearInt })
    console.log('📊 [USC Churn Member] Filters:', { line, year, month, userAllowedBrands: userAllowedBrands?.length || 0 })
    ...
    console.log(`📊 [USC Churn Member] Previous month (${prevMonth} ${prevYearInt}) active users found:`, prevData?.length || 0)
    ...
    console.log(`📊 [USC Churn Member] Current month (${month} ${yearInt}) active users found:`, currentData?.length || 0)
    ...
    console.log('📊 [USC Churn Member] Churn calculation:', {
    ...
    // Use prevMonthIndex from line 93 (already calculated)
    ...
    console.error('❌ Error calculating USC churn members:', error)
    return NextResponse.json({
      success: false,
...
async function fetchUserMinDates(rawData: any[], line: string | null, userAllowedBrands: string[] | null): Promise<Map<string, string>> {
  try {
    ...
    if (uniqueUsers.length === 0) {
      console.log('✅ [USC Churn Member] All users have valid first_deposit_date, no MIN date fetch needed')
      return new Map()
    }
    
    console.log(`🔍 [USC Churn Member] Fetching MIN dates for ${uniqueUsers.length} users with NULL first_deposit_date`)
    ...
    if (minDateError) {
      console.error('❌ [USC Churn Member] Error fetching MIN dates:', minDateError)
      return new Map()
    }
    ...
    console.log(`📊 [USC Churn Member] Fetched MIN dates for ${userMinDateMap.size} users`)
    
    return userMinDateMap
  } catch (error) {
    console.error('❌ [USC Churn Member] Error in fetchUserMinDates:', error)
    return new Map()
  }
}
