...
export async function GET(request: NextRequest) {
  try {
    console.log('🔍 Fetching unique values from blue_whale_usc for churn member slicers...')
    ...
    console.log('🔍 [USC Churn Member API] RAW brands from database:', lines)
    console.log('🔍 [USC Churn Member API] User allowed_brands:', userAllowedBrands)
    ...
    if (userAllowedBrands && userAllowedBrands.length > 0) {
      ...
      console.log('✅ [USC Churn Member API] Admin/Manager - ALL + brands:', finalLines)
    }
    ...
    console.log('✅ Blue_whale_usc churn member slicer options processed:', {
    ...
    console.error('❌ Error fetching blue_whale_usc churn member slicer options:', error)
    return NextResponse.json({ 
      success: false, 
      error: 'Internal server error while fetching blue_whale_usc churn member slicer options' 
    }, { status: 500 })
  }
}
