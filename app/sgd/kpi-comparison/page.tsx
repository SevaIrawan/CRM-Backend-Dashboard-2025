...
export default function KPIComparisonPage() {
  ...
  useEffect(() => {
    const fetchSlicerOptions = async () => {
      try {
        ...
        const response = await fetch('/api/sgd-kpi-comparison/slicer-options', {
          headers: {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/sgd-kpi-comparison/data?${params}`, {
        headers: {
          'x-user-allowed-brands': JSON.stringify(allowedBrands)
        }
      });
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `kpi-comparison-sgd-${selectedLine}-${periodAStart}-to-${periodBEnd}.csv`);
