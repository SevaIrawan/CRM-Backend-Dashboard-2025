...
export default function BrandComparisonPage() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const res = await fetch('/api/sgd-brand-comparison/slicer-options')
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/sgd-brand-comparison/data?${params}`)
        const json = await res.json()
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `brand-comparison-sgd-${periodAStart}-to-${periodBEnd}.csv`)
