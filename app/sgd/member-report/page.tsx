...
export default function SGDMemberReportPage() {
  ...
  const fetchSlicerOptions = async () => {
    try {
      ...
      const response = await fetch('/api/sgd-member-report/slicer-options', {
        headers: {
      ...
      if (result.success) {
        ...
        if (result.data.lines.length > 0) {
          ...
          setLine(firstOption)
          console.log('✅ [SGD Member Report] Auto-set line to first option:', firstOption)
        }
      }
    } catch (error) {
  ...
  const fetchMemberReportData = async () => {
    try {
      ...
      const response = await fetch(`/api/sgd-member-report/data?${params}`, {
        headers: {
          'x-user-allowed-brands': JSON.stringify(allowedBrands)
        }
      })
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await fetch('/api/sgd-member-report/export', {
        method: 'POST',
        headers: {
        ...
        <div className="deposit-container">
          {loading ? (
            <div className="loading-container">
              ...
              <p>Loading SGD member report data...</p>
            </div>
