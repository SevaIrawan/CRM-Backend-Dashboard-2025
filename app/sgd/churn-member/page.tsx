...
export default function SGDChurnMemberPage() {
  ...
  const fetchChurnMemberData = useCallback(async (pageNum: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/sgd-churn-member/data?${params}`, {
        headers: {
      ...
      console.error('Error fetching sgd-churn-member data:', error)
      setChurnMemberData([])
      setLoading(false)
    }
  ...
  useEffect(() => {
    if (!initialLoadDone && line && year && month && month !== 'ALL' && year !== 'ALL') {
      console.log('✅ [SGD Churn Member] Initial load with defaults:', { line, year, month })
  ...
  const fetchSlicerOptions = async () => {
    try {
      ...
      const response = await fetch('/api/sgd-churn-member/slicer-options', {
        headers: {
          'x-user-allowed-brands': JSON.stringify(allowedBrands)
        },
        cache: 'no-store'
      ...
      if (result.success) {
        ...
        if (result.data.defaults) {
          ...
          console.log('✅ [SGD Churn Member] Auto-set slicers to defaults:', result.data.defaults)
          ...
          console.log('✅ [SGD Churn Member] Auto-set slicers to fallback defaults')
        }
      }
    } catch (error) {
      console.error('Error fetching sgd-churn-member slicer options:', error)
    } finally {
  ...
  const handleDaysActiveClick = (userName: string, uniqueCode: string, userkey: string) => {
    console.log('🔍 [SGD Churn Member] Opening transaction history for user:', userName, uniqueCode, userkey)
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await fetch('/api/sgd-churn-member/export', {
        method: 'POST',
        headers: {
    ...
    <Layout customSubHeader={subHeaderContent}>
      <Frame variant="compact">
        <div className="deposit-container">
          {loading ? (
            <StandardLoadingSpinner message="Loading SGD Churned Members" />
            ...
            <div className="empty-container">
              ...
              <div className="empty-text">
                {!month || month === 'ALL' || !year || year === 'ALL' 
                  ...
                  : statusFilter !== 'ALL'
                    ? `No SGD churn member data found for status: ${statusFilter === 'NEW MEMBER' ? 'New Depositor' : 'Existing Member'}`
                    : 'No SGD churn member data found for the selected filters'}
              </div>
            </div>
          ) : (
            <>
...
function TransactionHistoryModal({
  ...
  const fetchTransactionHistory = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/sgd-churn-member/transaction-history?${params}`, {
