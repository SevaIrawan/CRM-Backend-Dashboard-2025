...
async function getSGDLastUpdateDate(year: string, month: string): Promise<number> {
  try {
    console.log(`🔍 [SGD Daily Average] Getting last update date for ${month} ${year}...`)
    ...
    if (data && data.length > 0 && data[0].date) {
      ...
      console.log(`✅ [SGD Daily Average] Last update date: ${dayOfMonth}`)
      return dayOfMonth
    }
...
async function getSGDCurrentMonthProgress(year: string, month: string): Promise<number> {
  ...
  if (year === currentYear && month === currentMonth) {
    console.log(`📅 [SGD Daily Average] CURRENT ongoing month detected: ${month} ${year}`)
    const lastUpdateDay = await getSGDLastUpdateDate(year, month)
    const currentDay = currentDate.getDate()
    const activeDays = Math.min(lastUpdateDay, currentDay)
    console.log(`📅 [SGD Daily Average] Current month active days: ${activeDays}`)
    return activeDays
  }
  
  // For ALL past months, use total days
  const totalDays = getDaysInMonth(year, month)
  console.log(`📅 [SGD Daily Average] Past/completed month: ${month} ${year}, total days: ${totalDays}`)
  return totalDays
}
...
async function calculateSGDDailyAverage(monthlyValue: number, year: string, month: string): Promise<number> {
  ...
  console.log(`📊 [SGD Daily Average] ${month} ${year}: ${monthlyValue} ÷ ${activeDays} = ${dailyAverage.toFixed(2)}`)
...
async function getSGDKPIData(year: string, month: string, line?: string): Promise<SGDKPIData> {
  try {
    console.log(`🔍 [SGD KPI] Getting KPI data for ${month} ${year} from MV...`)
    ...
    console.log(`✅ [SGD KPI] KPI data loaded for ${month} ${year}`)
    return kpiData
...
export async function calculateAllSGDDailyAverages(
  ...
  try {
    console.log('🔄 [SGD Daily Average] Calculating Daily Average for ALL SGD KPIs...')
    ...
    console.log('✅ [SGD Daily Average] All SGD KPIs Daily Average calculated')
...
export async function getAllSGDKPIsWithMoM(
  ...
  try {
    console.log('🔄 [SGD MoM] Calculating MoM for ALL SGD KPIs...')
    ...
    console.log('✅ [SGD MoM] All SGD KPIs with MoM calculated')
